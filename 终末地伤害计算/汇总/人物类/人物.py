...
character =["等级","星级","类型","武器", "攻击力", 
         "力量","敏捷","智识","意志",
         "战技倍率","连携技倍率","终结技倍率","源石技艺强度",
         "天赋1","天赋1效果强度","天赋1效果次数", "天赋2","天赋2效果强度","天赋2效果次数"]
...
class Character:
    ...
    def to_dict(self) -> Dict:
        # 修改：在序列化字典中添加 skill_multipliers 字段
        return {
            "name": self.name,
...
def save_library(lib: CharacterLibrary, filename: str = None):
    if filename is None:
...
def load_library(filename: str = None) -> CharacterLibrary:
    if filename is None:
        filename = os.path.join(SCRIPT_DIR, "characters.json")
    try:
